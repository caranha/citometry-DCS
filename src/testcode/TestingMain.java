		...
		try
		{
			...
			for(int c1 = 1; c1 < 11; c1++)
				for(int c2 = c1+1; c2 < 11; c2++)
				{
					...
					// selecting the data
					RealLabelledData train_data = train_c[c1].clone();
					RealLabelledData t2 = train_c[c2].clone();
					train_data.merge(t2);

					// Cluster 10 (normal) has no test data. So test_c[10] does not exist
					RealLabelledData test_data = test_c[c1].clone();
					if (c2 < 10)
					{
						t2 = test_c[c2].clone();
						test_data.merge(t2);	
					...
					RealLabelledData train_data_x = train_data.selectAttrib(attrib_select_x);
					...
					for (int k = 0; k < P.repeats; k++)
					{
						...
						LinProj P1 = new LinProj(train_data.attr_num);
						...
						RealLabelledData dproj = train_data.transform(P1);
						
						// calculate the best 5 attributes of the new projection
						int[] attrib_select_y = TestResult.bestAttributes(dproj, 5);
						RealLabelledData train_data_y = dproj.selectAttrib(attrib_select_y);
						...
						LinProj P2 = new LinProj(train_data.attr_num);
			...
			for(int c1 = 1; c1 < 11; c1++)
				for(int c2 = c1+1; c2 < 11; c2++)
					for(int c3 = c2+1; c3 < 11; c3++)			
					{
						...
						RealLabelledData train_data = train_c[c1].clone();
						RealLabelledData t2 = train_c[c2].clone();
						RealLabelledData t3 = train_c[c3].clone();
						train_data.merge(t2);
						train_data.merge(t3);
